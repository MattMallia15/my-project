...
- `portfolio/company_notes.csv` — running log of every conversation about specific tickers. Columns: `date, ticker, topic, key_facts, view_or_decision, next_step, sources`. **Append a row every time a company is discussed in any depth** — earnings prints, valuation checks, screens, order debates, news that materially affects a position. CSV is the source of truth (Excel opens it natively). When the user asks for "the Excel sheet" or "the workbook," regenerate `portfolio/company_notes.xlsx` from the CSV by running `python3 scripts/csv_to_xlsx.py` (already authored, uses openpyxl). Always regenerate the xlsx after appending rows so it stays in sync. When the user asks for "show me what we've said about X," read the CSV and present in a readable table — no need to open the xlsx.
- `portfolio/reports/daily/YYYY-MM-DD.md` — end-of-day market reports. **Generate when the user asks** "give me the daily" / "EOD report" / "what happened today." Template in `portfolio/reports/daily/README.md`. Six sections: top-line market, news drivers, portfolio moves+news, look-ahead, open decisions, FX cash policy check. Pull live prices via WebSearch; never quote from memory.
- `portfolio/reports/monthly/YYYY-MM.md` — monthly per-position deep dive. **Generate on/after month-end when asked** ("monthly review", "do the month"). Template in `portfolio/reports/monthly/README.md`. One full section per holding (size, P&L, thesis status, what changed, strategy alignment, decision for next month) plus portfolio-level summary, allocation snapshot, strategy/view check, open decisions, plan, and lessons learned.
